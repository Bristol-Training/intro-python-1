...
Python doesn't understand the meaning of words "Tuesday" and "Wednesday", they are just two chains of characters. The operator `>` will return `True` if the string `str1` comes after alphabetically than `str2`, and `<` will return `True` if the string `str1` comes before alphabetically than `str2`.
:::

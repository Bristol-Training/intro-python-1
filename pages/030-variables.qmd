---
title: Types of Data and Operators
---

Variables are names that refer to values or other objects. They act as memory containers for storing data values and are created when you assign a value to them using the assignment operator `=`. For example, `x = 5` assigns the integer value 5 to the variable `x`.
...
So far, all the code we've run is somewhat static. Variables get assigned a value in our code and every time we run it, the output will always be the same. We will see a lot more of this throughout this workshop, but for now we will introduce one more function that Python provides, [`input`](https://docs.python.org/3/library/functions.html#input).

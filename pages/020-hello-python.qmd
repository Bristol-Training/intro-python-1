...
This course will use a tool called *Notebooks* to run your Python code. Once Google Colab is open, to open a Notebook go to `File` > `New notebook in Drive`.

Once the notebook is launched, you will see a wide box with a `[ ]:` to the left. The box is an input cell where you type any Python code you want to run. For instance, type:

```{.python}
print("Hello, Python!")
```

Then, to run the cell click the button (▶) on the left or press Ctrl-Enter or Shift-Enter with the cursor inside. The output will appear below when the cell is run:
